...
int main()
{
	...
	while (selecao != '0')
	{
		...
		case '1': //Check-in.
			...
			do
			{ //Loopa enquanto o apartamento escolhido estiver ocupado, at� usuario dititar 0 para voltar ou at� quando o usu�rio digitar que n�o quer continuar.
				...
				{ //Garante que o apartamento seja valido (entre 1 e 14).
					...
					for (i = 0; i < 14; i++)  //Rotaciona por quarto do andar escolhido para verificar os quartos validos
						if (apartamentos[(andar - 1) * 14 + i].status == -1) //Checa se o quarto � valido
			...
			{												  //Checa se o apartamento esta ocupado (Diferente de -1).
				...
				if (selecao == 's' || selecao == 'S')
				{
					for (i = 0; i < apartamentos[apAtual].familia.membrosTotal; i++)
					{
						...
						if(apartamentos[apAtual].familia.membros[i].idade >= 18) //Checa se � menor de idade.
		...
		case '9': //Procurar por um hospede.
			...
			do
			{ //Loopa enquanto o usu�rio n�o escolher uma das op��es.
				scanf(" %c", &selecao);
